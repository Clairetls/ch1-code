...
provisioning$age_year<-provisioning$age
...
provaic<-DO(provisioning, 'prate_z',constant = '+newlifespan+BroodSize+nr_helpers+
            newstat+avg_invert+s(birthyear, bs="re")+s(WatchType,bs="re")+s(Observer, bs="re")',family=gaussian)

saveRDS(provaic, "provaic.rds")
