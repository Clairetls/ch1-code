...
reprosuccess$SexEstimate<-reprosuccess$sex  
reprosuccess$age_year<-reprosuccess$parent_age
reprosuccess$BirdID<-as.factor(reprosuccess$BirdID)
reprosuccess$SexEstimate<-as.factor(reprosuccess$SexEstimate)
...
#########
rsaic<-DO(reprosuccess, 'ars', constant='+newlifespan+minrepro+maxrepro+flno+newstat+
          s(birthyear, bs="re")',family=poisson)

saveRDS(rsaic, 'rsaic.rds')